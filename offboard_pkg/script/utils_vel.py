...
class Utils(object):
    def __init__(self, params):
        ...
        self.kf_p = 0.2  # 0.1是比较缓慢的朝上移动  0.12
        self.kf_vod = 20
    ...
    def DockingControllerFusion(self, pos_info, pos_i):
        ...
        wb1 = pos_info["mav_R"].T.dot(we1) #pos_info["mav_R"].T.dot(we1)
        ...
        rd = self.Euler_to_RotationMatrix(0, 0.1, 0)
        ...
        wb = (1 - self.change) * self.M.dot(wb1) + self.change * wb2
        ...
        self.Fpos = self.SaftyZ(self.pos_enable * n3.dot(self.propar_pos), 0.03) #saturation Fpos
        F_ref = self.hover + self.kf_p * n3.dot(dlt_v) + self.Fpos # self.pos_enable * n3.dot(self.propar_pos) 
        F = max(F_ref, self.hover - 0.1)
        F = self.SaftyZ(F, 0.60) #saturation F
        print("vod: {}".format(vod))
        ...
        # wb[2] = 0
        # F = self.hover + 0.01
        self.vel_body[0] = self.SaftyZ(self.wy_velx * wb[1], 10)
        self.vel_body[1] = self.SaftyZ(- self.wx_vely * wb[0],10)
        # self.vel_body[2] = self.SaftyZ(self.f_velz * (F - self.hover), 10)
        self.yaw_rate = wb[2] 
        self.vel_enu = pos_info["mav_R"].dot(self.vel_body)
        self.vel_enu[2] = self.SaftyZ(self.f_velz * (F - self.hover), 10)
        print("vel_enu: {}".format(self.vel_enu))
        print("vel_body: {}".format(self.vel_body))
        print("dlt_F: {}".format(F - self.hover))

        return [self.vel_enu[0], self.vel_enu[1], self.vel_enu[2], self.yaw_rate]
