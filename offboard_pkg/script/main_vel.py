...
if __name__=="__main__":
    ...
    while not rospy.is_shutdown():
       ...
       #command_rate.twist.angular.x > 0------------back command.twist.linear.x-------------forward
       #command_rate.twist.angular.y------------right    command.twist.linear.y------------left
       #command.twist.linear.z----------------right-to-left 
       #command_thrust.thrust----------------up    command.twist.linear.z-----------up
        if ch20 == 1:
            # sphere_control(pose_msg)
            if cnt % 10 == 0:
                print("ch20 is ok")
