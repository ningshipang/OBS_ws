...
class Utils(object):
    ...
    def DockingControllerFusion(self, pos_info, pos_i):
        ...
        rd = self.Euler_to_RotationMatrix(pos_info["mav_original_angle"][0], 0, 0)
        # rd = self.Euler_to_RotationMatrix(0, 0, 0)
        ...
        # pos_d = pos_info["Initial_pos"] #the position before entering offboard mode
        pos_d = np.array([pos_info["Initial_pos"][0], pos_info["Initial_pos"][1], pos_info["Initial_pos"][2]], dtype=np.float64)
        print("pos_d: {}".format(pos_info["Initial_pos"]))
